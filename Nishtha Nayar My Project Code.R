...
#Hypothesis 1: Higher the star Rating, higher the room rent

#Testing Hypothesis 1 with t-test
t.test(hotels$RoomRent, hotels$StarRating)

#Hypothesis 2: Higher hotel capacity means higher room rent

#Testing Hypothesis 2 with t-test
t.test(hotels$RoomRent, hotels$HotelCapacity)

#Forming regression models

model1 <- lm(RoomRent ~ StarRating + HotelCapacity + HasSwimmingPool, data = hotels)
summary(model1)

model2 <- lm(RoomRent ~ StarRating + HotelCapacity + HasSwimmingPool + IsTouristDestination, data = hotels)
summary(model2)

model3 <- lm(RoomRent ~ StarRating + HotelCapacity + HasSwimmingPool + IsTouristDestination + IsMetroCity + Airport + CityRank, data = hotels)
